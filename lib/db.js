...
function createPost(post, user_id) {
    return new Promise((resolve, reject) => {
        ...
        const stmt = db.prepare("INSERT INTO post (post_data, post_description, user_id) VALUES (?,?,?)");
        stmt.run([post.fecha, post.description, user_id],
            function (err) {
                if (err) {
                    reject(err);
                } else {
...
function getAllPostFromUsersFollowedBy(user_id) {
    return new Promise((resolve, reject) => {
        const query = `
            SELECT p.post_id, p.post_data, p.post_description, p.user_id, u.username
            FROM post p
